...
export const leagueSettingsSchema = z.object({
  qbCount: z.number().int().min(0).max(3),
  rbCount: z.number().int().min(0).max(4),
  wrCount: z.number().int().min(0).max(4),
  teCount: z.number().int().min(0).max(3),
  flexCount: z.number().int().min(0).max(3),
  kCount: z.number().int().min(0).max(2),
  defCount: z.number().int().min(0).max(2),
  benchCount: z.number().int().min(0).max(10),
